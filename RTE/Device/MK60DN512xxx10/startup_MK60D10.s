...
;expression can have any integer value from 0 to 31. The section is aligned on 
;a 2^(expression-byte boundary). For example, if expression is 10, the section
;is aligned on a 1KB boundary. This is not the same as the way that the ALIGN 
;directive is specified.
;
;sectionname:
;is the name to give to the section. Sections are independent,
...
;alphabetic character must be enclosed in bars or a missing section name error 
;is generated. For example, | 1_DataArea|. Certain names are conventional. For
;example, |.text| is used for code sections produced by the C compiler, or for
;code sections otherwise associated with the C library. 
;
;{attr}NOINIT:
;Indicates that the data section is uninitialized, or initialized to zero. It 
;contains only space reservation directives SPACE or DCB, DCD, DCDU, DCQ, DCQU,
;DCW, or DCWU with initialized values of zero. You can decide at link time 
;whether an area is uninitialized or zero-initialized.
;******************************************************************************
